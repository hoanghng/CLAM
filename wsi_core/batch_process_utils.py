...
def initialize_df(slides, seg_params, filter_params, vis_params, patch_params, 
    ...
    if isinstance(slides, pd.DataFrame):
        ...
        slide_ids = slides
    # normalize slide ids to strings and parse patient_id (format: <patient_id>-<scan_id>-<num>.svs)

    normalized = []
    patient_ids = []
    for s in slide_ids:
        if s is None:
            raise ValueError("Slide ID cannot be None")
        s = str(s).strip()
        # keep only the filename (remove any directory components)
        match = re.match(r'^(\d+)', s)
        if match:
            patient_ids.append(match.group(1))
        else:
            raise ValueError(f"Invalid slide ID format: {s}")
        
        normalized.append(s)
    slide_ids = [str(s) for s in slide_ids]

    # if input was a DataFrame, add a patient_id column (will be preserved when filling defaults)
    if isinstance(slides, pd.DataFrame):
        slides['patient_id'] = patient_ids

    default_df_dict = {'slide_id': slide_ids, 'patient_id': patient_ids, 'process': np.full((total), 1, dtype=np.uint8)}
